...
Data_boxes = numpy.array([['Principal Quantum Number n: ','entry',0,0], ['Allowed Values: 1,2,3,...','label',0,2],
                          ...
                          ['Opacity Shift: ','entry',14,0],['Allowed Values in [0,1]','label',14,2],['Use Plot Titles: ','check',14,3],
...
default_check_variables = [False,False,True,False,False,False,False,True,True,False,False,False]
...
while All_valid == False:  #the user can't escape the GUI sequence until all the variables are valid
   # Create a new window with the title 'Atomic Orbital Setup'
    ...
    title_flag = checks[7]      #if True plots the quantum numbers in the title of the figure
    if checks[8]:
        light_mode = 'white'    #used to determine if the plot background is white or black
    if checks[8] == False:
        light_mode = 'black'
    box_outline = checks[9]     #if True plots a box around the edges of the grid, if False does not
    nice_labels = checks[10]     #if True makes axis labels a multiple of 10, if False gives real values
    WF_Comp = checks[11]    #if True, plots the r, theta, and phi components of the wavefunction individually
...
def Mayavi_Volume(x,y,z,orbital,part):     #orbital is the array containing the orbital information (PhiReal or PhiImag)
    ...
    Plot_title = ' ' #default stance is not to give a title for the orbital
    if part == 'magnitude' and plot_mode == 'original' and title_flag:
    ...
    if part == 'real':  #using part to decide which orbital name goes on the plot
        if plot_mode == 'orbital' and title_flag:
            mlab.title('n='+ str(n) + ', l=' + str(l) + ', m=' + str(m) + ' Orbital', size=0.2)
        if plot_mode == 'probability density' and title_flag:
            mlab.title('n='+ str(n) + ', l=' + str(l) + ', m=' + str(m) + ' Probability Density', size=0.2)
        if plot_mode == 'original' and title_flag:
...
if WF_Comp:
    ...
    try:
        print("LaTeX Distribution found: using LaTeX for plot labels.")
